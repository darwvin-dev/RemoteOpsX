    ...
    if server.auth_type == "key" {
        if let Some(key) = &server.private_key_path {
            if !key.trim().is_empty() {
                args.push("-i".into()); args.push(key.clone());
                args.push("-o".into()); args.push("IdentitiesOnly=yes".into());
            }
        }
...
pub(crate) fn validate_tunnel(tunnel: &Tunnel) -> Result<(), TunnelValidationError> {
    ...
    if tunnel.local_port == 0 {
        return Err(TunnelValidationError::new("local_port", "local port must be between 1 and 65535"));
    }
    match tunnel.r#type.as_str() {
        "dynamic" => Ok(()),
        "local" | "remote" => {
            if tunnel.remote_host.as_deref().map_or(true, |host| host.trim().is_empty()) {
                return Err(TunnelValidationError::new("remote_host", "remote host is required"));
            }
            if tunnel.remote_port.map_or(true, |port| port == 0) {
                return Err(TunnelValidationError::new("remote_port", "remote port must be between 1 and 65535"));
            }
            Ok(())
        }
        other => Err(TunnelValidationError::new("type", format!("unknown tunnel type: {other}"))),
    }
}
...
impl TunnelManager {
    ...
    pub fn start_via(
        ...
        match tunnel.r#type.as_str() {
            "local" => {
                let remote_host = tunnel.remote_host.clone().unwrap_or_else(|| "127.0.0.1".into());
                let remote_port = tunnel.remote_port.ok_or_else(|| anyhow!("remote_port required for local forward"))?;
                args.push("-L".into());
                args.push(format!("{}:{}:{}:{}", local_host, tunnel.local_port, remote_host, remote_port));
            }
            "remote" => {
                let remote_host = tunnel.remote_host.clone().unwrap_or_else(|| "127.0.0.1".into());
                let remote_port = tunnel.remote_port.ok_or_else(|| anyhow!("remote_port required for remote forward"))?;
                args.push("-R".into());
                args.push(format!("{}:{}:{}:{}", local_host, tunnel.local_port, remote_host, remote_port));
            }
            "dynamic" => {
        ...
        let mut child = cmd.spawn().map_err(|error| anyhow!("failed to start tunnel: {error}"))?;
        for _ in 0..4 {
    ...
    pub fn active_ids(&self) -> Vec<String> {
        ...
        let mut alive = Vec::new(); let mut dead = Vec::new();
...
impl Drop for TunnelManager {
    fn drop(&mut self) {
        if let Ok(mut processes) = self.procs.lock() {
            for (_, child) in processes.iter_mut() { let _ = child.kill(); let _ = child.wait(); }
            processes.clear();
        }
    }
}
...
mod tests {
    ...
    fn tunnel(kind: &str) -> Tunnel {
        Tunnel { id: "tunnel-1".into(), server_id: "server-1".into(), r#type: kind.into(),
            local_host: Some("127.0.0.1".into()), local_port: 8080,
            remote_host: Some("127.0.0.1".into()), remote_port: Some(80),
            status: "pending".into(), created_at: String::new() }
    }

    fn server(auth_type: &str, key_path: Option<&str>) -> Server {
        Server { id: "server-1".into(), name: "test".into(), host: "example.com".into(), port: 22,
            ftp_port: None, rdp_port: None, vnc_port: None, username: "root".into(),
            protocols: vec!["ssh".into()], auth_type: auth_type.into(),
            private_key_path: key_path.map(|value| value.to_string()), tags: vec![], group_name: None,
            environment: "dev".into(), notes: None, created_at: String::new(), updated_at: String::new() }
    }
    ...
    fn password_auth_tunnel_disables_pubkey_so_agent_keys_cant_exhaust_maxauthtries() {
        let server = server("password", None); let mut args = Vec::new(); push_auth_args(&server, &mut args);
        assert!(has_opt(&args, "PubkeyAuthentication=no"));
    }

    #[test]
    fn key_auth_tunnel_still_uses_identities_only() {
        let server = server("key", Some("/home/user/.ssh/id_ed25519")); let mut args = Vec::new();
        push_auth_args(&server, &mut args); assert!(has_opt(&args, "IdentitiesOnly=yes"));
    ...
    fn rejects_invalid_tunnel_parameters() {
        let mut value = tunnel("local"); value.local_port = 0; assert!(validate_tunnel(&value).is_err());
        value.local_port = 8080; value.remote_port = None; assert!(validate_tunnel(&value).is_err());
        value.remote_port = Some(80); value.r#type = "invalid".into(); assert!(validate_tunnel(&value).is_err());
    }

    #[test]
    fn accepts_supported_tunnel_shapes() {
        assert!(validate_tunnel(&tunnel("local")).is_ok()); assert!(validate_tunnel(&tunnel("remote")).is_ok());
        let mut dynamic = tunnel("dynamic"); dynamic.remote_host = None; dynamic.remote_port = None;
